...
const exchangePoints = (req, res) => {
    ...
    if(!userId || !merchId || !merchPoints) {
        res.status(400).json({
            status: 'error',
            message: 'Invalid request. Please provide user id, merch id, and merch points'
        });
        return;
    }
    db.query('SELECT * FROM users WHERE id = ?', [userId], (error, results) => {
        ...
        db.query('UPDATE users SET total_points = ? WHERE id = ?', [total_points, userId], (error, results2) => {
            ...
            db.query('UPDATE merch SET stok = stok - 1 WHERE id = ?', [merchId], (error, results3) => {
                if(error){
                    res.status(500).json({
                        status: 'error',
                        message: 'Internal server error. Cannot update merch, please try again later',
                    });
                    console.log(error);
                    return;
                }
                ...
                db.query('INSERT INTO users_merch_redeem VALUES (?, ?, ?, NOW())', [id, userId, merchId], (error, results4) => {
                    if(error){
                        res.status(500).json({
                            status: 'error',
                            message: 'Internal server error. Cannot insert merch redeem by user, please try again later',
                        });
                        console.log(error);
                        return;
                    }
...
const addNotification = (req, res) => {
    ...
    [id, userId, title, message], (error, results) => {
        if(error){
            res.status(500).json({
                status: 'error',
                message: 'Internal server error. Cannot insert user notification, please try again later',
            });
            console.log(error);
            return;
        }
...
const getNotificationByUserId = (req, res) => {
    ...
    db.query('SELECT * FROM users_notifications WHERE user_id = ?',  [userId], (error, results) => {
        if(error){
            res.status(500).json({
                status: 'error',
                message: 'Internal server error. Cannot find user notification, please try again later',
            });
            console.log(error);
            return;
        }
...
const getAllMerch = (req, res) => {
    db.query('SELECT * FROM merch', (error, results) => {
        if(error){
            res.status(500).json({
                status: 'error',
                message: 'Internal server error. Cannot get data from merch, please try again later',
            });
            console.log(error);
            return;
        }
...
const getMerchRedeemedByUserId = (req, res) => {
    ...
    db.query('SELECT b.name as name, b.image as image, a.date as date FROM users_merch_redeem a JOIN merch b ON a.merch_id = b.id JOIN users c ON a.user_id = c.id WHERE a.user_id = ?',
    [userId], (error, results) => {
        if(error){
            res.status(500).json({
                status: 'error',
                message: 'Internal server error. Cannot get data, please try again later',
            });
            console.log(error);
            return;
        }
