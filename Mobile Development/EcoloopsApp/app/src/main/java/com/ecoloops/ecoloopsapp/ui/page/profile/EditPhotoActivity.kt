            ...
            uploadImageRequest.enqueue(object : Callback<UploadPhotoResponse> {
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        startActivity(Intent(this@EditPhotoActivity, EditProfileActivity::class.java))
                        finish()
                        Toast.makeText(this@EditPhotoActivity, responseBody?.message, Toast.LENGTH_SHORT).show()

                    } else {
                        val errorBody = response.errorBody()?.string()
                        val jsonObject = JSONObject(errorBody.toString())
                        val message = jsonObject.getString("message")
                        CustomAlertDialog(this@EditPhotoActivity, message, R.drawable.custom_error).show()
                ...
                override fun onFailure(call: Call<UploadPhotoResponse>, t: Throwable) {
                    binding.submitEditButton.isEnabled = true
                    binding.submitEditButton.text = "Upload Story"
                    Toast.makeText(this@EditPhotoActivity, t.message, Toast.LENGTH_SHORT).show()
                }
            })

        } else {
            Toast.makeText(this@EditPhotoActivity, "Silakan masukkan berkas gambar terlebih dahulu.", Toast.LENGTH_SHORT).show()
        }
